...
const ExpensesSummary: React.FC<Props> = ({ listDetails, listItems }) => {
    ...
    return (
        <>
            {modalOpen && (
                <Modal>
                    <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-[rgba(56,189,248,0.14)] text-2xl text-[#38bdf8]">↻</div>
                    ...
                    <div className="mt-6 grid gap-3 md:grid-flow-col">
                        ...
                        <button onClick={handleConfirmSettle} className="order-1 rounded-full bg-[#38bdf8] px-8 py-3 font-bold text-[#05131c] md:order-2">Confirm paid</button>
                    </div>
                </Modal>
            )}
            ...
            <div className="relative overflow-hidden rounded-[22px] border border-white/[0.08] p-[28px_30px]"
                ...
                {!listDetails.archived && (
                    <button onClick={() => setModalOpen(true)}
                            className="rounded-full bg-[#38bdf8] px-[26px] py-3 text-[14px] font-bold text-[#05131c]">Settle up</button>
                )}
            </div>
        </>
    );
...
export const PersonSpendCards: React.FC<Props> = ({ listDetails, listItems }) => {
    const t = useTotals(listDetails, listItems);
    const rows = [
        { name: t.user1Name, sum: t.user1Sum, pct: t.user1Pct, color: NEIL, tint: 'rgba(56,189,248,0.16)' },
